    ...
    public partial class bugs : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (proj != null)
            {
                ...
                {
                    connection.Open();
                    var command = new MySqlCommand("Select * From projects WHERE `pname`='" + proj + "';", connection);
                ...
                {
                    connection.Open();
                    var command = new MySqlCommand("Select * From bugs WHERE `project`='"+proj+"';", connection);
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
